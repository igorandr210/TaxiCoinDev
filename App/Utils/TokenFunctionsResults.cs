...
namespace App.Utils
{
    public class TokenFunctionsResults<TResult, TPattern> where TPattern : IControllerPattern
    {
        ...
        public static TransactionReceipt InvokeByTransaction(TPattern req, string funcName, params object[] funcParametrs)
        {
            Crypto.DecryptTwoStringsAndGetContractFunctions(out string senderAddress, req.Sender, out string password, req.Password, req.PassPhrase, out ContractFunctions contractFunctions);

            return contractFunctions.CallFunctionByNameSendTransaction(senderAddress, password, funcName, funcParametrs).Result;
        }
        ...
        public static TransactionReceipt InvokeByTransaction(TPattern req, string funcName)
        {
            ...
            return contractFunctions.CallFunctionByNameSendTransaction(senderAddress, password, funcName, null).Result;
        }
    }
}
